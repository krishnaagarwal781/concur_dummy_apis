...
from models.models import DataUpdateRequest, DataDeletionRequest, DPOAssignment, DPOAudit, DPOComplianceReport, DPOFeedback
...
async def approve_data_update_request(request_id: str):
    result = await db["data_update_requests"].update_one({"_id": ObjectId(request_id)}, {"$set": {"status": "approved", "processed_at": datetime.utcnow()}})
...
async def reject_data_update_request(request_id: str, explanation: str):
    result = await db["data_update_requests"].update_one({"_id": ObjectId(request_id)}, {"$set": {"status": "rejected", "processed_at": datetime.utcnow(), "explanation": explanation}})
    if result.matched_count:
...
async def approve_data_deletion_request(request_id: str):
    result = await db["data_deletion_requests"].update_one({"_id": ObjectId(request_id)}, {"$set": {"status": "approved", "processed_at": datetime.utcnow()}})
    if result.matched_count:
...
async def reject_data_deletion_request(request_id: str, explanation: str):
    result = await db["data_deletion_requests"].update_one({"_id": ObjectId(request_id)}, {"$set": {"status": "rejected", "processed_at": datetime.utcnow(), "explanation": explanation}})
    if result.matched_count:
...
async def dpo_compliance_report() -> DPOComplianceReport:
    ...
    total_approved = await db["data_update_requests"].count_documents({"status": "approved"})
    total_rejected = await db["data_update_requests"].count_documents({"status": "rejected"})
    compliance_percentage = (total_approved / total_requests) * 100 if total_requests > 0 else 0
