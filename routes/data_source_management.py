...
@data_source_router.get(
    "/view-data-source/{source_id}", tags=["Data Source Management"]
)
...
@data_source_router.put(
    "/update-data-source/{source_id}", tags=["Data Source Management"]
)
async def update_data_source(source_id: str, source: UpdateDataSource):
    if not ObjectId.is_valid(source_id):
        raise HTTPException(status_code=400, detail="Invalid data source ID")
    result = await db["data_sources"].update_one(
        {"_id": ObjectId(source_id)}, {"$set": source.dict(exclude_unset=True)}
    )
...
@data_source_router.delete(
    "/delete-data-source/{source_id}", tags=["Data Source Management"]
)
...
@data_source_router.put(
    "/archive-data-source/{source_id}", tags=["Data Source Management"]
)
async def archive_data_source(source_id: str):
    if not ObjectId.is_valid(source_id):
        raise HTTPException(status_code=400, detail="Invalid data source ID")
    result = await db["data_sources"].update_one(
        {"_id": ObjectId(source_id)}, {"$set": {"status": "archived"}}
    )
    if result.matched_count:
        return {"status": "success"}
    raise HTTPException(status_code=404, detail="Data source not found")
...
async def export_data_source(
    format: str = Query("json", description="Format to export the data (json, csv)")
):
    sources = await db["data_sources"].find().to_list(100)
    if format == "json":
        return sources
        ...
        return Response(
            ...
            media_type="text/csv",
            headers={"Content-Disposition": "attachment; filename=data_sources.csv"},
        )
...
@data_source_router.post(
    "/validate-data-source/{source_id}", tags=["Data Source Management"]
)
...
@data_source_router.post(
    "/sync-data-source/{source_id}", tags=["Data Source Management"]
)
